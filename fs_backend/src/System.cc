...
: Node("cpu_node_tmp"),
  ...
  output_path_depth_(std::move(output_path_depth)) {
    ...
    // --- Basic sanity: need at least one image, mate. ---
    ...
    // --- Read first image to establish working resolution. ---
    ...
    // --- Keep resize positive; round to nearest; never let dims hit zero. ---
    ...
    // --- Laplacian kernel must be odd and >= 1. ---
    ...
    for (int lvl = 0; lvl < num_pyramid_; ++lvl) {
        ...
        local_sharpness_img_[lvl].create(height_ / scale, width_ / scale, CV_64F);
        local_sharpness_img_[lvl].setTo(0);
    }

    // --- Quick param dump for sanity. ---
    ...
    // Use as many threads as you're comfy with (tune to your box).
...
void System::pyDown(cv::Mat& img, std::vector<cv::Mat>& pyramid) {
    ...
    for (int i = 1; i < num_pyramid_; ++i) {
        pyrDown(pyramid[i - 1], pyramid[i]); // shrink each level by ~2
    }
}
...
void System::coarse_to_fine(std::vector<cv::Mat>& pyramid, int img_id) {
    ...
    // Reset local sharpness buffers per level
    for (int lvl = 0; lvl < num_pyramid_; ++lvl) {
        local_sharpness_img_[lvl].setTo(0);
    }

    // 1) Per-level Laplacian + variance over patches (working at that level’s resolution)
    for (int lvl = 0; lvl < num_pyramid_; ++lvl) {
        cv::Mat lap;
        ...
        for (int x = 0; x < W; ++x) {
            for (int y = 0; y < H; ++y) {
                // Patch sizes (left as-is; callers can tweak via params)
                ...
                const cv::Mat patch = lap(roi);

                cv::Scalar mean, stddev;
                cv::meanStdDev(patch, mean, stddev);
                const double variance = stddev[0] * stddev[0];

                // IMPORTANT: write at the level’s own resolution (no x*w or y*w here).
    ...
    for (int lvl = 0; lvl < num_pyramid_; ++lvl) {
        ...
        for (int x = 0; x < W; ++x) {
            for (int y = 0; y < H; ++y) {
                ...
                // accumulate contribution (you can add level weights if you like)
                tmp_sharpness_img.at<double>(yn, xn) += v;
            }
        }
    }

    // 3) Update global maxima and index image (which source image won per pixel)
    #pragma omp parallel for collapse(2)
...
// Just a thin wrapper in case we want to swap the operator later.
...
void System::run() {
    ...
    for (int img_id = 0; img_id < static_cast<int>(path_to_imgs_.size()); ++img_id) {
        // Progress callback (nice to have in the UI)
